 ...
 * </pre>
 * <p/>
 * The algorithm is modeled on the recursive depth-first search. By varying the functions SELECT-UNASSIGNED-VARIABLE and
 ...
 * An example of solution strategy is backtracking algorithm. You can note that when an assignment is done, 
 * the fireStateChanged(assignment, csp) method is triggered, so that another class listening to this one can trace, 
...
public class BacktrackingStrategy extends SolutionStrategy {
        ...
        if (assignment.isComplete(csp.getVariables())) {
            ...
            for (Object value : orderDomainValues(var, assignment, csp)) {
                ...
                if (assignment.isConsistent(csp.getConstraints(var))) {
                    Domain2 info = inference(var, assignment, csp);
                    if (!info.isEmpty())
     ...
     */
    protected Domain2 inference(Variable var, Assignment assignment,
                                          CSP csp) {
        return new Domain2().compactify();
    }
}
